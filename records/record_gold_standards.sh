...
python /project/cmcwhite/github/old_form_pcm/protein_complex_maps/protein_complex_maps/corum/complex_merge.py --cluster_filename $COMPLEXES --output_filename gold_standards/Positives_merged06_size30_rmLrg_rmSub.txt --merge_threshold 0.6 --complex_size_threshold 30 --remove_largest --remove_large_subcomplexes

# Split complexes to training and test complexes
python /project/cmcwhite/github/protein_complex_maps/protein_complex_maps/preprocessing_util/complexes/split_complexes.py --input_complexes gold_standards/Positives_merged06_size30_rmLrg_rmSub.txt

# Alphabetize training and test interactions. 
for ppi in gold_standards/*ppis.txt
    do 
        python /project/cmcwhite/github/protein_complex_maps/protein_complex_maps/features/alphabetize_pairs.py --feature_pairs $ppi --outfile ${ppi%.txt}.ordered --sep '\t'
    done


# Get all ID pairs from the feature matrix (remove header with tail)
awk -F',' '{print $1}' feature_matrix/wheat_feature_matrix.csv |  tail -n +2 > gold_standards/labels
